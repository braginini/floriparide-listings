...
Ext.define('Deft.ioc.DependencyProvider', {
  ...
  */
  ...
  resolve: function(targetInstance, targetInstanceConstructorArguments, identifier) {
    ...
    if (this.getFn() != null) {
      ...
      if (targetInstanceConstructorArguments) {
        fnArguments = [targetInstance, identifier].concat(Ext.toArray(targetInstanceConstructorArguments));
      } else {
        fnArguments = [targetInstance, identifier];
      }
...
Ext.define('Deft.ioc.Injector', {
  ...
  */
  ...
  configure: function(configuration) {
    ...
    Ext.Object.each(configuration, function(identifier, config) {
      ...
      if (Ext.String.startsWith(identifier,'$')) {
        config.regex = new RegExp(identifier.substring(1));
      }
      provider = Ext.create('Deft.ioc.DependencyProvider', config);
      this.providers[identifier] = provider;
      newProviders[identifier] = provider;
      if (provider.regex) {
        this.regexProviders.push(provider);
      }
    }, this);
    Ext.Object.each(newProviders, function(identifier, provider) {
      if (provider.getEager()) {
        ...
        provider.resolve(null, null, identifier);
      }
    }, this);
  },
  /**
  ...
  */
  ...
  resolve: function(identifier, targetInstance, targetInstanceConstructorArguments) {
    ...
    provider = this.providers[identifier];
    if (provider == null) {
      for (i=0; i<this.regexProviders.length;i++) {
        if (this.regexProviders[i].regex.test(identifier)) {
          provider = this.regexProviders[i];
          break;
        }
      }
    }
    if (provider != null) {
      return provider.resolve(targetInstance, targetInstanceConstructorArguments, identifier);
    } else {
      Ext.Error.raise({
