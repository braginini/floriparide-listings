...
def recreate_index(index_name, doc_type):
...
def delete_index(index_name):
    ...
    logging.info("delete index %s" % index_name)


def create_index(index_name, doc_type):
    with open(os.path.join('es_configs', index_name + '_index.json')) as index_file, \
            open(os.path.join('es_configs', index_name + '_mapping.json')) as mapping_file:
        es.indices.create(index=index_name, body=json.load(index_file))
        es.indices.put_mapping(index=index_name, doc_type=doc_type, body=json.load(mapping_file))
        logging.info("create index %s" % index_name)
...
def update_index(project_id):
    ...
    project = project_dao.get_by_string_id(project_id)
    ...
    other_branches = branch_dao.get_by_attrs_rubrics(attribute_history_set, rubric_history_set,
                                                     branch_history_map.keys())
    ...
    for k, v in branch_history_map.items():
        ...
        if operation == 'D':
            ...
            data['rubrics'] = [format_rubric(rubrics, k) for k in v['data']['draft'].get('rubrics')]
            ...
            if curr_attributes:
                curr_attributes = [format_attribute(attributes, key) for key in v['data']['draft'].get('attributes')]
    ...
    if other_branches:
        for b in other_branches:
            ...
            data['rubrics'] = [format_rubric(rubrics, key) for key in b['data'].get('rubrics')]

            curr_attributes = b['draft'].get('attributes')
            if curr_attributes:
                curr_attributes = [format_attribute(attributes, key) for key in b['draft'].get('attributes')]
            data['attributes'] = curr_attributes
            action = {
                '_op_type': 'index',
                '_index': config.ES.INDEX,
...
def format_point(geometry):
    if geometry:
        ...
        if pnt and pnt['lat'] and pnt['lng']:
            # we use lat lon to support ES geo point type
            return dict(lat=float(pnt['lat']), lon=float(pnt['lng']))


def format_rubric(rubrics, branch_rubric_entry):
    rubric_id = str(branch_rubric_entry['id'])
    return dict(names=rubrics[rubric_id]['data']['names'], id=rubric_id)


def format_attribute(attributes, branch_attribute_entry):
...
if __name__ == '__main__':
    ...
    @click.argument('name')
    def cli_create_index(name):
        create_index(name, "branch")


    @cli.command(name='recreate_index')
    @click.argument('name')
    def cli_recreate_index(name):
        recreate_index(name, "branch")

    @cli.command(name='delete_index')
    @click.argument('name')
    def cli_delete_index(name):
        delete_index(name)

    @cli.command(name='update_index')
    @click.argument('project_id')
    def cli_update_index(project_id):
